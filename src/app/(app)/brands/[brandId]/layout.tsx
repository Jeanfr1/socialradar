...
export default async function BrandLayout({ children, params }: { children: React.ReactNode; params: Promise<{ brandId: string }> }) {
  ...
  return (
    <div>
      {brand.isDemo ? <DemoBanner scope={`${brand.name} is a demo brand with fixture data. No real accounts are connected in this view.`} /> : null}
      <div className="mb-2 flex flex-wrap items-center gap-2 text-sm text-ink-2">
        ...
        {brand.archivedAt ? <Pill tone="warning">Archived</Pill> : null}
        <Pill>{role}</Pill>
        <span>
          Times in {brand.timezone} ({zoneLabel(new Date(), brand.timezone)})
        </span>
      </div>
